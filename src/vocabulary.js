...
class Vocabulary {
    constructor(terms = []) {
        if (!(terms instanceof Array) && !(terms instanceof Set)) throw new Error('Whoops! Terms must be either an Array or a Set');
     ...
     */
    add(terms) {
        if (typeof terms !== 'string' && !(terms instanceof Array) && !(terms instanceof Set)) throw new Error('Whoops! Terms must be either a string, Array or Set');
        if (typeof terms === 'string') {
            terms = [terms]
        } else if (terms instanceof Set) terms = Array.from(terms);
        terms.forEach(term => {
     ...
     */
    indexOf(term) {
        if (!this._terms.has(term)) return -1
